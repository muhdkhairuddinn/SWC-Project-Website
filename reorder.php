...
if (isset($_GET['id'])) {
    ...
    if ($copy_order) {
        ...
        if ($copy_items) {
            echo "<script>alert('Order has been reordered successfully.'); window.location='http://localhost/opos2/index.php?page=my_orders';</script>";
        } else {
            echo "<script>alert('Failed to reorder items.'); window.location='http://localhost/opos2/index.php?page=my_orders';</script>";
        }
    } else {
        echo "<script>alert('Failed to create reorder.'); window.location='http://localhost/opos2/index.php?page=my_orders';</script>";
    }
}
?>