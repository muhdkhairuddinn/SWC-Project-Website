...
Class Action {
	...
	function save_order(){
		extract($_POST);
		$user_id = $_SESSION['login_user_id']; // Pastikan user_id diambil dari session
	
		$data = " user_id = '$user_id' "; // Tambah user_id
		...
		if($save){
			$id = $this->db->insert_id; // ID pesanan yang baru dimasukkan
			...
			while($row = $qry->fetch_assoc()){
				...
				if($save2){
					...
					die("❌ Error memasukkan item pesanan: " . $this->db->error);
				}
			}
			
			return 1; // Pesanan berjaya
		} else {
			die("❌ Error menyimpan pesanan: " . $this->db->error);
		}
	}
